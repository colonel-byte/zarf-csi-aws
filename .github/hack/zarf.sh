...
for flavor in "${FLAVOR[@]}"; do
  ...
  for component in "${COMPONENTS[@]}"; do
    ...
    zarf dev find-images --flavor $flavor --skip-cosign $component 2>/dev/null > .direnv/$flavor/out.yml
    ...
    for type in "${COMPONENTS_TYPES[@]}"; do
      ...
      export yq_index=$(yq "$yq_search" $component/zarf.yaml)
      ...
      yq ea -i "$yq_images | $yq_charts | $yq_update | $yq_comments" $component/zarf.yaml .direnv/$flavor/out.yml
    done
  done
done
